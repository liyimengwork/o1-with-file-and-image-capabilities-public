...
st.sidebar.markdown("This version is for local testing; it saves markdown files to local folder")
st.sidebar.markdown("Our online demo is available on [Streamlit](xx) and does not collect any user data")
...
def save_markdown(prompt, response):
    ...
    pathlib.Path("Response").mkdir(exist_ok=True)
...
def save_uploaded_file(uploaded_file):
    # **Use the stored timestamp instead of generating a new one**
    date_time = st.session_state.get('date_time')
    if not date_time:
        # Fallback in case date_time is not set
        now = datetime.now()
        date_time = now.strftime("%Y-%m-%d-%H%M%S")
    
    original_filename = uploaded_file.name
    filename = f"Response/{date_time}-{original_filename}"
...
def save_feedback(feedback, email):
    ...
    # Generate filename using the stored date_time
    filename = f"Response/{date_time}-feedback.md"
    ...
    if user_input or uploaded_files:
        ...
        if uploaded_files:
            for uploaded_file in uploaded_files:
                uploaded_file_filename = save_uploaded_file(uploaded_file)
                # st.success(f"Uploaded file saved to {uploaded_file_filename}")
    else:
...
with st.sidebar.form(key='feedback_form'):
    ...
    if feedback_submit:
        if feedback.strip():
            feedback_file = save_feedback(feedback, contact_email)
            with open(feedback_file, "r", encoding="utf-8") as f:
                st.sidebar.download_button(
                    label="Download Feedback",
                    data=f.read(),
                    file_name=os.path.basename(feedback_file),
                    mime="text/markdown"
                )
            st.sidebar.success("Thank you for your feedback!")
            # {{ Ensure that submitting feedback does not clear generated content }}
        else:
